...
class YiiBase
{
	...
	public static function createComponent($config)
	{
		...
		// FIXME: B? sung param d? construct class
		else if (isset($config['constructParams'])) {
			$class=new ReflectionClass($type);
			$object=call_user_func_array(array($class,'newInstance'), $config['constructParams']);
		}	
		else
			$object=new $type;
		...
		if(($path=self::getPathOfAlias($alias))!==false)
		{
			if($isClass)
			{
				if($forceInclude)
				{
					if(is_file($path.'.php'))
						require($path.'.php');
					// FIXME: �� c?p nh?t d? autoload Zend khi import
