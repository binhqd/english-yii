...
500);var S={type:"slideMouseOver",index:z,data:a};j.isFunction(c.settings.slideMouseOver)&&c.settings.slideMouseOver.call(this,S)},function(){c.settings.navigationArrows&&c.settings.fadeNavigationArrows&&u.find(".navigation-arrows a").stop().animate({opacity:0},500);c.settings.slideshowControls&&c.settings.fadeSlideshowControls&&u.find(".slideshow-controls").stop().animate({opacity:0},500);var S={type:"slideMouseOut",index:z,data:a};j.isFunction(c.settings.slideMouseOut)&&c.settings.slideMouseOut.call(this,
S)});B.click(function(){var S={type:"slideClick",index:z,data:a};j.isFunction(c.settings.slideClick)&&c.settings.slideClick.call(this,S)});M=Math.floor(Math.min(a.width,c.settings.width)/h);X=Math.floor(Math.min(a.height,c.settings.height)/e);ga=a.width>c.settings.width?Ea(d,a.width,c.settings.width):0;d=a.height>c.settings.height?Fa(d,a.height,c.settings.height):0;R=a.width<c.settings.width?Math.floor((c.settings.width-a.width)/2):0;var ha=a.height<c.settings.height?Math.floor((c.settings.height-
...
height:a.height,overflow:"hidden",position:"absolute"});B.html(b).appendTo(ia)}N&&N.appendTo(ia);if(t=="random")t=ba(["autoHorizontal","autoVertical","rightToLeft","leftToRight","topToBottom","bottomToTop"]);i={};var F,K;switch(t){case "autoHorizontal":F="left";K=z>L?c.settings.width:-c.settings.width;break;case "autoVertical":F="top";K=z>L?c.settings.height:-c.settings.height;break;case "rightToLeft":F="left";K=c.settings.width;break;case "leftToRight":F="left";K=-c.settings.width;break;case "topToBottom":F=
"top";K=-c.settings.height;break;case "bottomToTop":F="top";K=c.settings.height;break;default:F="left";K=z>L?c.settings.width:-c.settings.width}B.css(F,K);i[F]=0;B.animate(i,{duration:r,easing:x,complete:function(){B.appendTo(A);ia.remove();ta()},step:function(S){N&&N.css(F,S-K)}})}else{for(t=0;t<h;t++)for(r=0;r<e;r++){x=j('<div class="slice"></div>').css({left:t*M+R,top:r*X+ha,width:M,height:X,opacity:v,visibility:"hidden"}).data({hPos:t,vPos:r}).appendTo(B);a.path&&x.css({"background-image":"url("+
...
"leftBottom","horizontalAlternative","verticalAlternative"]);g=Qa(Ga,f,h,e);h=g.length;for(t=0;t<h;t++)Ra(g[t],t,h,i,m,n,s,o,k,q)}i={type:"transitionStart",index:z,data:a};j.isFunction(c.settings.transitionStart)&&c.settings.transitionStart.call(this,i)}function ta(){var a=w[z],b=a.properties.alignType,d=Ea(b,a.width,c.settings.width);b=Fa(b,a.height,c.settings.height);var g=a.properties.effectType,h=a.properties.htmlDuringTransition;E=false;if(g!="simpleSlide"){B.find(".slice").each(function(){clearTimeout(j(this).data("timer"));
...
break;case "default":d.animate({top:parseInt(d.css("height"))},m,function(){a&&b.remove()})}}function sa(){var a=w[z].properties.slideshowDelay||c.settings.slideshowDelay;c.settings.timerAnimation&&Ta(a);ja=0;ka=(new Date).getTime();G&&clearTimeout(G);G=setTimeout(function(){if(c.settings.slideshowDirection=="next")aa();else c.settings.slideshowDirection=="previous"&&$()},a)}function fa(){G&&clearTimeout(G);c.settings.timerAnimation&&Ua()}function ea(){G&&clearTimeout(G);ja+=(new Date).getTime()-
ka}function pa(){var a=w[z].properties.slideshowDelay||c.settings.slideshowDelay;ka=(new Date).getTime();G&&clearTimeout(G);G=setTimeout(function(){if(c.settings.slideshowDirection=="next")aa();else c.settings.slideshowDirection=="previous"&&$()},a-ja)}function Ta(a){var b=document.createElement("canvas"),d=Math.max(c.settings.timerStrokeWidth1,c.settings.timerStrokeWidth2)+c.settings.timerRadius*2,g=d/2,h=Math.PI/180,e=0,f,i,m=c.settings.timerStrokeOpacity1,n=c.settings.timerStrokeOpacity2,s=Y(c.settings.timerStrokeColor1).red,
...
if(c.settings.thumbnailOrientation=="horizontal"){p.css({width:d*Math.min(a,c.settings.visibleThumbnails),height:g,top:A.outerHeight(true)+c.settings.navigationThumbnailsDistance});y.css({width:d*Math.min(a,c.settings.visibleThumbnails),height:g});H=w.length*d}else{p.css({width:d,height:g*Math.min(a,c.settings.visibleThumbnails),left:A.outerWidth(true)+c.settings.navigationThumbnailsDistance});y.css({width:d,height:g*Math.min(a,c.settings.visibleThumbnails)});H=w.length*g}if(c.settings.fadeNavigationThumbnails){p.css({opacity:0});
...
T-1){J++;W(J)}else W(0)}).appendTo(e);if(c.settings.thumbnailOrientation=="horizontal"){p.css("width",d*Math.min(a,c.settings.visibleThumbnails)+2*(b.outerWidth()+c.settings.thumbnailArrowsDistance));y.css("left",Math.round((p.outerWidth()-y.outerWidth())/2));b.css({left:0,top:Math.round((p.outerHeight()-b.outerHeight())/2)});h.css({right:0,top:Math.round((p.outerHeight()-h.outerHeight())/2)})}else{p.css("height",g*Math.min(a,c.settings.visibleThumbnails)+2*(b.outerHeight()+c.settings.thumbnailArrowsDistance));
y.css("top",Math.round((p.outerHeight()-y.outerHeight())/2));b.css({top:0,left:Math.round((p.outerWidth()-b.outerWidth())/2)});h.css({bottom:0,left:Math.round((p.outerWidth()-h.outerWidth())/2)});b.addClass("vertical");h.addClass("vertical")}if(c.settings.fadeThumbnailArrows){e.hide();p.hover(function(){e.stop(false,true).fadeIn(c.settings.thumbnailArrowsShowDuration)},function(){e.stop(false,true).fadeOut(c.settings.thumbnailArrowsHideDuration)})}}if(c.settings.thumbnailButtons){var f=j('<div class="buttons"></div>').appendTo(p),
...
i*T;b.width=i}f.css(b);if(c.settings.thumbnailOrientation=="horizontal"){p.css("height",g+f.outerHeight(true)+c.settings.thumbnailButtonsDistance);a.left=(p.outerWidth()-f.outerWidth())/2;a.top=y.outerHeight()+c.settings.thumbnailButtonsDistance}else{p.css("width",d+f.outerWidth(true)+c.settings.thumbnailButtonsDistance);a.top=(p.outerHeight()-f.outerHeight())/2;a.left=y.outerWidth()+c.settings.thumbnailButtonsDistance}f.css(a);if(c.settings.fadeThumbnailButtons){f.hide();p.hover(function(){f.stop(false,
...
c.settings.thumbnailTooltip)j(document).bind("mousemove",function(m){O=m.pageX;P=m.pageY});c.settings.thumbnailScrollbar&&Xa();c.settings.thumbnailMouseScroll&&Ha();c.settings.thumbnailMouseWheel&&Ya();if(c.settings.thumbnailOrientation=="horizontal"){i=parseInt(p.css("top"))+p.outerHeight();i=u.outerHeight()<i?i-u.outerHeight():0;u.css("height",u.outerHeight()+i)}else{i=parseInt(p.css("left"))+p.outerWidth();i=u.outerWidth()<i?i-u.outerWidth():0;u.css("width",u.outerWidth()+i)}}function Va(a){if(a=
...
a.css("left",-9999)})}function W(a){var b=c.settings.thumbnailOrientation=="horizontal"?y.outerWidth():y.outerHeight(),d=a*b;J=a;if(J==T-1)d=H-b;ca();D.animate(c.settings.thumbnailOrientation=="horizontal"?{left:-d}:{top:-d},{duration:c.settings.thumbnailScrollDuration,easing:c.settings.thumbnailScrollEasing,step:function(){la()},complete:function(){va()}});b=p.find(".buttons");b.find(".select").removeClass("select");b.find("a").eq(a).removeClass("over").addClass("select")}function Ha(){c.settings.thumbnailMouseScroll=
true;var a=0,b=false,d=y.offset().top,g=y.offset().left,h=y.outerWidth(true),e=y.outerHeight(true),f=c.settings.thumbnailOrientation=="horizontal"?h:e,i=c.settings.thumbnailOrientation=="horizontal"?g:d,m=c.settings.thumbnailOrientation=="horizontal"?"left":"top",n=parseInt(D.css(m));wa=setInterval(function(){if(!b){b=true;d=y.offset().top;g=y.offset().left;h=y.outerWidth(true);e=y.outerHeight(true)}if(O>g&&O<g+h&&P>d&&P<d+e){a=Math.round((-((H-f+200)*(((c.settings.thumbnailOrientation=="horizontal"?
O:P)-i)/f))+100-n)*c.settings.thumbnailMouseScrollSpeed/100);n+=a}else if(Math.abs(a)>0.1){a*=c.settings.thumbnailMouseScrollEase/100;n+=a}else a=0;if(Math.abs(a)>0){if(n>=0)n=0;else if(n<=f-H)n=f-H;la();D.css(m,n)}},30)}function Ya(){c.settings.thumbnailMouseWheel=true;var a=0,b=c.settings.thumbnailMouseWheelReverse?-1:1,d,g=c.settings.thumbnailOrientation=="horizontal"?"left":"top",h=c.settings.thumbnailOrientation=="horizontal"?y.outerWidth(true):y.outerHeight(true);y.bind("mousewheel",function(e,
f){e.preventDefault();if(!xa){ca();xa=true;a=d=parseInt(D.css(g));ya=setInterval(function(){if(Math.abs(a-d)>0.5){d+=(a-d)*(c.settings.thumbnailMouseWheelSpeed/100);la();D.css(g,d)}else va()},30)}a+=b*f*10;if(a>=0)a=0;else if(a<=h-H)a=h-H})}function Xa(){function a(){i=(c.settings.thumbnailOrientation=="horizontal"?O:P)-g.offset()[k]-m;b()}function b(){if(i<0)i=0;else if(i>parseInt(g.css(o))-parseInt(h.css(o)))i=parseInt(g.css(o))-parseInt(h.css(o));da&&h.css(k,i);n=i/(parseInt(g.css(o))-parseInt(h.css(o)));
if(!ma){ca();ma=true;s=parseInt(D.css(k));za=setInterval(function(){if(Math.abs(Ia()+n)>0.0010){var q=(-n*(H-parseInt(y.css(o)))-s)*c.settings.thumbnailScrollbarEase/100;s+=q;la();D.css(k,s)}else ma&&va()},30)}}var d=j("<div></div>").appendTo(p),g=j('<div class="track"></div>').appendTo(d),h=j('<div class="thumb"></div>').appendTo(g),e=j('<div class="left"></div>').appendTo(d),f=j('<div class="right"></div>').appendTo(d),i=0,m,n=0,s=0,o=c.settings.thumbnailOrientation=="horizontal"?"width":"height",
k=c.settings.thumbnailOrientation=="horizontal"?"left":"top";if(c.settings.thumbnailOrientation=="horizontal"){d.addClass("scrollbar");d.css({top:y.outerHeight()+c.settings.thumbnailScrollbarDistance,left:(p.outerWidth()-parseInt(d.css("width")))/2});p.css("height",p.outerHeight()+c.settings.thumbnailScrollbarDistance+d.outerHeight())}else{d.addClass("scrollbar-vertical");d.css({left:y.outerWidth()+c.settings.thumbnailScrollbarDistance,top:(p.outerHeight()-parseInt(d.css("height")))/2});p.css("width",
p.outerWidth()+c.settings.thumbnailScrollbarDistance+d.outerWidth())}if(c.settings.fadeThumbnailScrollbar){d.hide();p.hover(function(){d.stop(false,true).fadeIn(c.settings.thumbnailScrollbarShowDuration)},function(){d.stop(false,true).fadeOut(c.settings.thumbnailScrollbarHideDuration)})}h.bind("mousedown",function(q){q.preventDefault();m=(c.settings.thumbnailOrientation=="horizontal"?O:P)-h.offset()[k];da=true;j(document).bind("mousemove",a)});j(document).bind("mouseup",function(){if(da){da=false;
j(document).unbind("mousemove",a)}});e.bind("click",function(){i=parseInt(h.css(k))-c.settings.scrollbarArrowScrollAmount;b()});f.bind("click",function(){i=parseInt(h.css(k))+c.settings.scrollbarArrowScrollAmount;b()})}function Ia(){var a=c.settings.thumbnailOrientation=="horizontal"?"width":"height";return parseFloat(D.css(c.settings.thumbnailOrientation=="horizontal"?"left":"top"))/(H-parseInt(y.css(a)))}function ca(){wa&&clearInterval(wa);if(ya){xa=false;clearInterval(ya)}if(za){ma=false;clearInterval(za)}D&&
D.is(":animated")&&D.stop()}function va(){ca();c.settings.thumbnailMouseScroll&&Ha()}function la(){if(c.settings.thumbnailScrollbar&&!da){var a=u.find(".track"),b=a.find(".thumb");positionProp=c.settings.thumbnailOrientation=="horizontal"?"left":"top";sizeProp=c.settings.thumbnailOrientation=="horizontal"?"width":"height";value=-Ia()*(parseInt(a.css(sizeProp))-parseInt(b.css(sizeProp)));value>0&&b.css(positionProp,value)}}function Qa(a,b,d,g){var h=[],e,f,i=0;switch(b){case "randomPattern":for(d=
...
g)for(e=k++;e<n;e++)h.push(l(a,e,g-k));if(q==d)for(f=k++;f<s;f++)h.push(l(a,k-1,f))}h.reverse();break;default:for(d=[];a.length;){i=Math.floor(Math.random()*a.length);d.push(a[i]);a.splice(i,1)}m=d.length;for(b=0;b<m;b++)h[b]=d[b]}return h}function l(a,b,d){return j.grep(a,function(g){return g.data("hPos")==b&&g.data("vPos")==d})[0]}function Y(a){var b=parseInt(a.substring(1,3),16),d=parseInt(a.substring(3,5),16);a=parseInt(a.substring(5,7),16);return{red:b,green:d,blue:a}}var u=j(oa),c=this,z=-1,
L=-1,w=[],B=null,N=null,E=false,V=false,qa=false,Z=false,G=0,ua=0,J=0,T=0,C="",ja=0,ka=0,wa,ya,xa=false,za,da=false,ma=false,O,P,H,p,y,D,A,na=["htmlDuringTransition","alignType","effectType","sliceDelay","sliceDuration","sliceEasing","horizontalSlices","verticalSlices","slicePattern","slicePoint","slideStartPosition","slideStartRatio","sliceFade","captionSize","captionPosition","captionShowEffectDuration","captionShowEffectEasing","captionHideEffectDuration","captionHideEffectEasing","captionShowEffect",
"captionHideEffect","captionLeft","captionTop","captionWidth","captionHeight","captionShowSlideDirection","captionHideSlideDirection","captionBackgroundColor","captionBackgroundOpacity","slideshowDelay","slideMask","linkTarget","simpleSlideDirection","simpleSlideDuration","simpleSlideEasing","lightboxDefaultWidth","lightboxDefaultHeight","lightboxTheme","lightboxOpacity"];this.settings=j.extend({},j.fn.advancedSlider.defaults,U);(function(){if(c.settings.xmlSource){u.empty();j.ajax({type:"GET",url:c.settings.xmlSource,
